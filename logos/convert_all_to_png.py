#!/usr/bin/env python3
"""
转换所有版本的 SVG logo 到 PNG
"""

import subprocess
import os
from pathlib import Path

# 基础路径
BASE_DIR = Path("/Users/zhujin05/data/seo/code/seo-nav/logos")

# 所有版本和配色
VERSIONS = {
    "v1-abstract": ["warm-orange", "orange-red", "orange-gold"],
    "v2-silhouette": ["warm-orange", "orange-red", "orange-gold"],
    "v3-texture": ["warm-orange", "orange-red", "orange-gold"],
}

# PNG尺寸
SIZES = {
    "logo-1024.png": 1024,
    "logo-512.png": 512,
    "favicon-192.png": 192,
    "favicon-64.png": 64,
    "favicon-32.png": 32,
    "favicon-16.png": 16,
}

def convert_svg_to_png(svg_path, png_path, size):
    """使用librsvg的rsvg-convert转换SVG到PNG"""
    try:
        cmd = [
            "rsvg-convert",
            "-w", str(size),
            "-h", str(size),
            "-f", "png",
            "-o", str(png_path),
            str(svg_path)
        ]
        subprocess.run(cmd, check=True, capture_output=True)
        return True
    except (subprocess.CalledProcessError, FileNotFoundError):
        print(f"  ⚠️  转换失败")
        return False

def main():
    print("🎨 开始转换所有版本 SVG 到 PNG...\n")

    total_converted = 0

    for version, colors in VERSIONS.items():
        print(f"\n📦 处理 {version}/")
        for color in colors:
            color_dir = BASE_DIR / version / color
            svg_file = color_dir / "logo.svg"

            if not svg_file.exists():
                print(f"  ❌ SVG文件不存在: {svg_file}")
                continue

            print(f"  📁 {color}/")

            for png_name, size in SIZES.items():
                png_file = color_dir / png_name

                if convert_svg_to_png(svg_file, png_file, size):
                    print(f"    ✅ {png_name} ({size}x{size})")
                    total_converted += 1
                else:
                    print(f"    ❌ {png_name} 转换失败")

    print(f"\n\n🎉 完成！共转换 {total_converted} 个PNG文件")

if __name__ == "__main__":
    main()
